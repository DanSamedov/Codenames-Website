class CardManager {
  ...
  initCards(data = {}) {
    ...
    cards.forEach((card) => {
      ...
      if (
        action ===
          window.GameConstants.WEBSOCKET.ACTIONS.REVEAL_GUESSED_CARDS &&
    ...
    if (
      action === window.GameConstants.WEBSOCKET.ACTIONS.REVEAL_GUESSED_CARDS &&
  ...
  getChosenCount() {
    return document.querySelectorAll(
      `[data-is-chosen="${window.GameConstants.CARD_STATES.TRUE}"]`
    ).length;
  }
  ...
  updateSelectedCards(selectedCards) {
    ...
    cards.forEach((card) => {
      const isGuessed =
        card.dataset.isChosen === window.GameConstants.CARD_STATES.GUESSED;
      card.dataset.isChosen = isGuessed
        ? window.GameConstants.CARD_STATES.GUESSED
        : window.GameConstants.CARD_STATES.FALSE;
      card.style.border = "";
    });
