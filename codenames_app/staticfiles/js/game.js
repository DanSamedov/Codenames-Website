const gameId = JSON.parse(
    document.getElementById('game-id-data').textContent,
);

const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
const host = window.location.host;
const gameSocket = new WebSocket(
    `${protocol}//${host}/ws/game/${gameId}/`
);

const initializedKey = `game_${gameId}_initialized`;

const currentUserName = window.roomConfig.currentUser;
const currentUserTeam = window.roomConfig.currentTeam;
const currentUserLeader = window.roomConfig.currentRole;
...
gameSocket.onmessage = function (e) {
    ...
    if (
        ...
        document.querySelectorAll('[data-is-chosen]').forEach(card => {
